...
def add_or_update_cart(request):
    ...
    if request.method == 'POST':
        try:
            ...
            # 1. MultipleObjectsReturned error theke bachte transaction use kora bhalo
            with transaction.atomic():
                # Jodi age theke duplicate row thake (ja tomar error log-e dekha geche), segulo delete koro
                duplicates = OrderCart.objects.filter(customer=customer, product_id=product_id, is_order=False)
                if duplicates.count() > 1:
                    duplicates.delete()
                
                # 2. update_or_create logic
                cart_item, created = OrderCart.objects.update_or_create(
                    ...
                    defaults={
                        'quantity': quantity,
                        'is_active': True if int(quantity) > 0 else False, 
                    }
                )
                # Jodi age theke thaka row "is_active=False" thake, setake force update koro
                if not created:
                    cart_item.is_active = True if int(quantity) > 0 else False
                    cart_item.quantity = quantity
                    cart_item.save()
                    
                # 3. Quantity 0 hole row delete na kore is_active false kora (soft delete)
                if quantity <= 0:
                    cart_item.is_active = False
                    cart_item.save()
                    isRemoved = True
                else:
                    cart_item.is_active = True # Jodi purone inactive row thake setake active kora
                    cart_item.save()

            # 4. Summary data calculation
            ...
            return JsonResponse({
                ...
                'isRemoved': isRemoved,
                'item_price': cart_item.total_amount if hasattr(cart_item, 'total_amount') else 0,
...
@login_required
def cart(request):
    # Ekhane extra query-r dorkar nai, context processor automatically 'cart_items' pathiye debe
